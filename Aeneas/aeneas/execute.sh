...
function which_program_only_pipelines {
	...
	then
		...
		cd ..
		cd ..
		#echo "QUESTA è LA DIRECTORY DEI FILE : ${PWD}"
		for file1 in assets/aeneas-assets/audio/*.mp3
		do
			local arrAudios=("${arrAudios[@]}" "$file1")
		done
		...
		do 
			...
			echo "STO ESEGUENDO IL CALCOLO"
			asset_testo=${arrTexts[$j]}
			#newassetname=assets/aeneas-assets/aeneastext.xhtml
			local conf="task_language=eng|os_task_file_format=smil|os_task_file_smil_audio_ref=audio.mp3|os_task_file_smil_page_ref=page.xhtml|is_text_type=unparsed|is_text_unparsed_id_regex=f[0-9]+|is_text_unparsed_id_sort=numeric"
			executeApplication="python -m aeneas.tools.execute_task     ${asset_audio}     ${asset_testo}     ${conf}     map.smil"
...
function upload_to_s3 {
	...
	echo -e "Questa è l'attuale directory: $PWD"
	echo -e "Questo è il sistema operativo: $(uname -a)"
	# capture time it takes to transfer to the S3 bucket
	start=$(date +%s.%N)
	echo "Questa è l'attuale directory: $PWD"
	python3 s3Upload.py $predictions $predictions
	end=$(date +%s.%N)
...
function save_to_files {
	...
	#echo "FUNZIONE SAVE TO FILES"
	#echo "DIRECTORY CORRENTE : ${PWD}"
	#cat results.txt >> /mnt/results/cloudresult.txt
	echo ${metricsValues[@]} >> arrresult.txt
